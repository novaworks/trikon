...
function nova_product_quick_view_button_v2() {
  ...
  if( 1 ==  Nova_OP::getOption('shop_product_quickview_button') ):
    echo '
      <a href="#" class="nova_product_quick_view_btn" data-product-id="' . $product->get_id() . '" rel="nofollow"><svg class="trikon-plus"><use xlink:href="#trikon-plus" xmlns:xlink="http://www.w3.org/1999/xlink"></use></svg></a>
    ';
  endif;
}
endif;
...
function kitify_add_toolbar_close(){
    if(wc_get_loop_prop('kitify_loop_allow_extra_filters')){
      ...
      if(!empty($per_page_array)):
        ...
        echo '<label>'.esc_html__( 'Show','trikon' ).'</label>';
        echo '<ul>';
        $active_a = '';
