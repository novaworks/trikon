		...
		<div class="nova-blog-posts post-display-block">
			...
			<div class="nova-blog-posts__content<?php if(has_post_thumbnail()): ?> has-thumbnail<?php endif?>">
					...
					if( $excerpt_limit ) {
						if($excerpt_length > 0){
								echo sprintf(
										...
										trikon_excerpt(intval( $excerpt_length ))
								);
						}
					}else {
						echo sprintf(
				...
				<div class="nova-blog-posts__readmore-wrap">
					<a class="nova-blog-posts__readmore link-animation-icon" href="<?php echo(esc_url(get_permalink())); ?>"><?php esc_html_e( 'Read more', 'trikon') ?><i class="inova ic-arrow-right"></i></a>
				</div>
			</div>
		</div>
</article>
